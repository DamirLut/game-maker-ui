  ...
  return (
    <div className={Style.window} ref={ref}>
      <div
        ref={refHeader}
        className={cn(
          Style.window__header,
          active && Style.window__header__active,
        )}
      >
        {props.collapsible && <button type="button"></button>}
        <span>{props.title}</span>
      </div>
      <div
        className={cn(Style.window__body, active && Style.window__body__active)}
        ref={bodyRef}
      >
        <div className={cn(Style.window__body__wrapper)}>{props.children}</div>
      </div>
    </div>
  );
}
